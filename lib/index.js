...
const name = "archived-conversations";
...
async function readPreview(sessionQuery, sessionId) {
	try {
		...
		const events = snapshot !== null && typeof snapshot === "object" && Array.isArray(snapshot.events) ? snapshot.events : [];
		const messages = [];
...
function sessionIdFrom(url) {
	...
	const value = new URL(url, "http://dsh.local").searchParams.get("sessionId");
...
function apply(ctx) {
	ctx.webServer.register({
		...
		handler: (req, res) => {
			...
			readPreview(sessionQuery, sessionId).then((result) => {
				sendJson(res, result);
			}, () => {
				sendJson(res, {
					ok: false,
