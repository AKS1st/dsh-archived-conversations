/**
 * dsh-archived-conversations — browser half (static bundle, plain JS).
 *
 ...
 *
 * Packaged as a static client bundle: `window.__ModuleLoader__.load(...)`
 * with `require('react')` for React and plain fetch for the host route.
 */
  ...
  window.__ModuleLoader__.load({
    ...
    factory(require) {
      ...
      function fetchPreview(sessionId) {
        return fetch(`${API_PREVIEW}?sessionId=${encodeURIComponent(sessionId)}`, { cache: 'no-store' })
          .then((res) => res.json())
      ...
      function ArchivedAction(props, translate) {
        ...
        const rows = React.useMemo(() => {
          ...
          for (const id of archivedIds) {
            ...
            result.push({
              id,
              title: summary !== undefined && summary.displayTitle !== '' ? summary.displayTitle : id,
              updatedAt: summary !== undefined ? summary.updatedAt : 0,
            })
          }
        ...
        const selected = selectedId === null ? undefined : rows.find((row) => row.id === selectedId)
        const messages = preview !== null && preview.ok === true && Array.isArray(preview.messages)
          ? preview.messages
          : []
        const previewError = preview !== null && preview.ok !== true
